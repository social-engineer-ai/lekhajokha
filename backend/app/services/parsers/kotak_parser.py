...
# Kotak format:
# Columns: Sl. No | Transaction Date | Value Date | Description | Chq / Ref No. | Debit | Credit | Balance
...
class KotakParser(BaseBankParser):
    ...
    def _map_columns(self, header: list[str | None]) -> dict[str, int]:
        ...
        for i, cell in enumerate(header):
            ...
            elif ("transaction" in c or "tran" in c) and "date" in c:
                col_map["txn_date"] = i
            elif "date" in c and "txn_date" not in col_map and "value_date" not in col_map:
                col_map["txn_date"] = i
            elif "description" in c or "particular" in c or "narration" in c:
                col_map["description"] = i
            elif "chq" in c or "ref" in c:
                col_map["cheque"] = i
            elif ("debit" in c or "withdrawal" in c or "dr" in c) and "balance" not in c and "debit" not in col_map:
                col_map["debit"] = i
            elif ("credit" in c or "deposit" in c or "cr" in c) and "balance" not in c and "credit" not in col_map:
                col_map["credit"] = i
            elif "balance" in c:
                col_map["balance"] = i
